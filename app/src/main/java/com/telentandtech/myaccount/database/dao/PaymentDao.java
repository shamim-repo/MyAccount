...
public interface PaymentDao {
    ...
    @Query("SELECT COUNT(*) AS totalCount, SUM(CASE WHEN payment_status = 1 THEN 1 ELSE 0 END)"+
                  " AS paidCount, SUM(CASE WHEN payment_status = 0 THEN 1 ELSE 0 END) AS Count " +
            "FROM payments WHERE uid=:uid")
    PaidUnpaidCountResult getPaidUnpaidCount(String uid);
    ...
    @Query("SELECT payment_month AS month,SUM(CASE WHEN payment_status = 1 THEN 1 ELSE 0 END) AS paid, " +
            ...
            "FROM payments WHERE uid=:uid GROUP BY payment_month")
    List<PaidUnpaidByMonth> getPaidDueCountByMonth(String uid);
